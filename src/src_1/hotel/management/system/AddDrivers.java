package src_1.hotel.management.system;

import javax.swing.*;
import java.awt.event.*;
import java.sql.*;

public class AddDrivers extends JFrame implements ActionListener {
    JTextField t1, t2, t3, t4, t5, t6, t7;
    JButton save;

    AddDrivers() {
        setLayout(null);

        JLabel l1 = new JLabel("Name");
        l1.setBounds(30, 30, 100, 30);
        add(l1);
        t1 = new JTextField();
        t1.setBounds(150, 30, 150, 30);
        add(t1);

        JLabel l2 = new JLabel("Age");
        l2.setBounds(30, 70, 100, 30);
        add(l2);
        t2 = new JTextField();
        t2.setBounds(150, 70, 150, 30);
        add(t2);

        JLabel l3 = new JLabel("Phone");
        l3.setBounds(30, 110, 100, 30);
        add(l3);
        t3 = new JTextField();
        t3.setBounds(150, 110, 150, 30);
        add(t3);

        JLabel l4 = new JLabel("Car Name");
        l4.setBounds(30, 150, 100, 30);
        add(l4);
        t4 = new JTextField();
        t4.setBounds(150, 150, 150, 30);
        add(t4);

        JLabel l5 = new JLabel("Car Number");
        l5.setBounds(30, 190, 100, 30);
        add(l5);
        t5 = new JTextField();
        t5.setBounds(150, 190, 150, 30);
        add(t5);

        JLabel l6 = new JLabel("Location");
        l6.setBounds(30, 230, 100, 30);
        add(l6);
        t6 = new JTextField();
        t6.setBounds(150, 230, 150, 30);
        add(t6);

        JLabel l7 = new JLabel("License Number");
        l7.setBounds(30, 270, 100, 30);
        add(l7);
        t7 = new JTextField();
        t7.setBounds(150, 270, 150, 30);
        add(t7);

        save = new JButton("Save");
        save.setBounds(150, 320, 100, 40);
        save.addActionListener(this);
        add(save);

        setSize(400, 400);
        setLocation(400, 100);
        setVisible(true);
    }

    public void actionPerformed(ActionEvent ae) {
        try {
            conn c = new conn();
            String query = "INSERT INTO driver(name, age, phone, car_name, car_number, location, license_number) VALUES(?,?,?,?,?,?,?)";
            PreparedStatement ps = c.c.prepareStatement(query);
            ps.setString(1, t1.getText());
            ps.setInt(2, Integer.parseInt(t2.getText()));
            ps.setString(3, t3.getText());
            ps.setString(4, t4.getText());
            ps.setString(5, t5.getText());
            ps.setString(6, t6.getText());
            ps.setString(7, t7.getText());
            ps.executeUpdate();
            JOptionPane.showMessageDialog(null, "Driver Saved Successfully");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}